...
4. [ROUTE_SYSTEM.md](./ROUTE_SYSTEM.md) — implemented route coexistence, publication gates, shared templates, SEO, and route verification.
5. [ARCHITECTURE.md](./ARCHITECTURE.md) — route, content, Server Component, client island, media, dependency, and SEO boundaries.
6. [DEVELOPMENT_PLAN.md](./DEVELOPMENT_PLAN.md) — controlled-rewrite strategy, phased delivery, approval gates, budgets, risks, and definition of done.
...
The server-first route layer is tracked through
[GitHub Issue #9](https://github.com/wildanniam/portofolio-wildan/issues/9) and
documented in [ROUTE_SYSTEM.md](./ROUTE_SYSTEM.md). The public `/` composition
is still the preserved legacy site; `/work` and `/contact` now exercise the V1
shell while unpublished case studies and `/moments` continue to fail closed.

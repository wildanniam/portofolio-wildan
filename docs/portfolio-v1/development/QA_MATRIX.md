...
- a lead image that has already completed in a failed state remains static when
  its project is revisited rather than reattaching dead load/error listeners;
- a delayed leave-frame cleanup cannot remove an overlay after the user has
  re-entered the active ScrollTrigger range;
- live reduced-motion and Save-Data changes clean and restore exactly one
  controller/overlay, while a desktop coarse pointer never loads the GSAP module;
- repeated eligibility threshold crossings keep one listener pair and one
  controller rather than accumulating global or root-owned state.
...
- the private four-mode Moments fixture renders lead, evidence, portrait, and
  contact-sheet compositions at 1440, 768, 640, and 390 pixels; mobile sources
  switch below 640 pixels and the contact sheet changes from two columns to one;
- moment records without ready media expose `data-has-media="false"` and use the
  prose-width layout instead of reserving an empty image column.

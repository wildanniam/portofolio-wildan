...
1. Current product outcome.
2. The pre-transaction decision gap.
3. Wildan contribution contact sheet with team boundary.
4. Address → features → inference → community cross-check → verdict flow.
5. Standalone analysis versus transaction-confirmation context.
6. System architecture with testnet/mainnet labels.
7. Canister, test, extension, and public-distribution proof.
8. WCHL result and real team moment.
9. Where the 2025 build stands and what Wildan would improve next.

### Nova AI Wallet

1. Wallet-friction map.
2. Team, challenge, and verified role.
3. Prompt → intent → tool/action plan.
4. Active agent architecture and signing boundary.
5. Real generative evidence card and sanitized payload.
6. Explicit wallet confirmation.
7. External model boundary and output.
8. Paylink as clearly labeled prototype material.
9. Exact award outcome and real photograph.
10. What the sprint taught Wildan and what he would improve next.

### PayGate

1. Human subscription flow versus machine request flow.
2. Source API → 402 → payment → protected endpoint.
3. Provider registration and secret protection.
4. Negative and positive upstream-verification probes.
5. Real testnet payment and credit proof.
6. Escrow accounting and signed withdrawal.
7. Operational request-state ledger.
8. Security-hardening issue-to-control strip.
9. Testnet, delivery, refund, and mainnet boundaries.

### Quorum

1. Multi-party event-settlement problem.
2. Real event/checkout product frame.
3. Locked collaborator split before publish.
4. Preflight → XDR inspection → Freighter approval → finality → persistence.
5. QuorumCore and QuorumPassNFT invariants.
6. Purchase/claim → pass → resource → check-in → withdrawal.
7. Indexer and public evidence model, including the current hosted-feed iteration.
8. Settlement-versus-MoneyGram-cash-out correction from issue #27/PR #28.
9. Tests, CI, QA, and six successful testnet transactions.
10. Proven-versus-unproven boundary.
...
- evidence caption and provenance template;
- commit/diff contact-sheet template;
- test, trace, and transaction redaction template;
...
- Figma Community and React-library assets are researched only in the later asset phase.
- Every external resource requires a recorded license and attribution decision.
- Community assets may establish a technique or production shortcut; they must not replace Wildan's authentic project evidence or become the portfolio concept.
- Generated imagery must never impersonate a real product screenshot, award, person, event, test, transaction, partner, or user result.
- Demo/seeded data is labeled in captions and cannot be used as traction evidence.
- Personal and team photographs remain unpublished until credit and consent are ready.
- Original photographs stay outside the workspace/repository; only cropped, redacted, EXIF-stripped, optimized, and explicitly approved derivatives enter `public/media`.
- Broken or incomplete live states can appear as honest engineering evidence, but never as an unlabeled polished outcome.

...
type NarrativeBlockProps = PropsWithChildren<{
  id?: string;
  label?: string;
}>;

type SourceLinkProps = PropsWithChildren<{
  href?: string;
  title?: string;
}>;
...
function NarrativeBlock({
  ...
  id,
  label,
}: NarrativeBlockProps & { component: string }) {
  return (
    <section
      aria-label={label}
      data-case-study-component={component}
      id={id}
    >
      {children}
    </section>
...
function SourceLink({ href, children, title }: SourceLinkProps) {
  ...
  return (
    <ApprovedLink href={href} title={title}>
      {children}
    </ApprovedLink>
  );
}
...
function isApprovedHref(href: string): boolean {
  ...
  if (href.startsWith("#") && href.length > 1 && !href.slice(1).includes("#")) {
    return true;
  }
  if (/^mailto:[^@\s]+@[^@\s]+\.[^@\s]+(?:\?[^\s]*)?$/i.test(href)) {
    return true;
  }
...
function ApprovedLink({
  ...
  href,
  title,
}: Pick<AnchorHTMLAttributes<HTMLAnchorElement>, "children" | "href" | "title">) {
  if (!href || !isApprovedHref(href)) {
    throw new TypeError("Case-study links require an approved static URL.");
  }

  return (
    <a data-case-study-component="SourceLink" href={href} title={title}>
      {children}
    </a>
  );
}
